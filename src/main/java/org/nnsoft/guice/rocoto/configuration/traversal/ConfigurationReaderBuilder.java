...
public abstract class ConfigurationReaderBuilder {
     ...
     * {@inheritDoc}
     */
    public final boolean accept(File configurationFile) {
        boolean accepted = match(this.configurationFilePattern, configurationFile.getPath(), true);
        if (this.logger.isLoggable(Level.FINE)) {
            this.logger.fine(String.format("File '%s' %s accepted\n", configurationFile, accepted ? "" : "NOT"));
        }
        return accepted;
    }
